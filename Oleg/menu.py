...
class Circle():
    ...
    def draw(self,root):
        ...
        pygame.draw.circle(root,self.color,(self.x+400,self.y),20)
        root.blit(pygame.font.Font(None, 22).render('V'+str(self.v), True, (0, 0, 0)),(self.x-7+400,self.y-7))
...
def window():
    ...
    root.blit(practice, (500, 250))
    if posM[0]>=500 and posM[0]<=615 and posM[1]>=250 and posM[1]<=270:
        pygame.draw.line(root,(255,255,255),(500,270),(612,270),2)
...
while run:
    ...
    if posM[0]>=500 and posM[0]<=585 and posM[1]>=200 and posM[1]<=220 and presM[0]==1:
        if theory.theorys():
            break
    if posM[0]>=500 and posM[0]<=615 and posM[1]>=250 and posM[1]<=270 and presM[0]==1:
